 ...
 * TealGroq Quickstart
 *
 * Demonstrates a guarded Groq client with input guardrails, low-latency timing,
...
import { GROQ_PRICING, TealGroq } from 'tealtiger/providers/groq';
...
function formatSeconds(seconds: number | undefined): string {
  return seconds == null ? 'not reported' : `${(seconds * 1000).toFixed(1)} ms`;
}
...
async function main() {
  ...
  budgetManager.createBudget({
    name: 'Groq Quickstart Daily Budget',
    limit: 5.0,
    period: 'daily',
  ...
  const client = new TealGroq({
    apiKey: process.env.GROQ_API_KEY || 'your-groq-api-key',
    agentId: 'groq-quickstart-agent',
    model: MODEL,
    guardrailEngine,
    costTracker,
    budgetManager,
  ...
  console.log('--- Basic Groq request ---');
  ...
  const basic = await client.chat.completions.create({
    ...
    messages: [
      ...
      {
        role: 'user',
        content: 'In one sentence, what does TealTiger add around Groq calls?',
      },
    ],
    max_tokens: 120,
  });
  ...
  console.log(`Measured latency: ${elapsedMs.toFixed(1)} ms`);
  console.log(`Groq reported total time: ${formatSeconds(basic.usage.total_time)}`);
  console.log(`Groq queue time: ${formatSeconds(basic.usage.queue_time)}`);
  ...
  const costSummary = await costStorage.getSummary(today, new Date(), 'groq-quickstart-agent');
...
main().catch((error) => {
  console.error('Groq quickstart failed:', error);
  process.exit(1);
});
