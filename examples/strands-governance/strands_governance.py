...
Usage:
    ...
    agent = Agent(
        tools=[...],
        hooks=[TealTigerGovernanceHook(mode="enforce", policies=[...])],
    )
"""
...
class TealTigerGovernanceHook(HookProvider):
    ...
    Args:
        mode: "observe" (log only), "monitor" (log + warn), "enforce" (block)
    ...
    def evaluate_tool_call(self, event: BeforeToolCallEvent) -> None:
        ...
        print(
            f"[TealTiger] {status} | {tool_name} | "
            f"{elapsed_ms:.2f}ms | {correlation_id[:8]}"
        )
    ...
    def report(self) -> dict[str, Any]:
        ...
        return {
            ...
            "pii_findings_total": sum(len(d.pii_detected) for d in self._audit_trail),
        }
