 ...
 * 
 * This example shows how to use the AgentGuard SDK in a simple JavaScript application
 */

const { AgentGuard } = require('../dist/index');

async function basicExample() {
  console.log('🚀 AgentGuard SDK - Basic Usage Example');
  console.log('=====================================');
  ...
  try {
    ...
    console.log('\n2. Safe Web Search');
    const searchResult = await agentGuard.executeTool(
      'web-search',
    ...
    const systemResult = await agentGuard.executeTool(
      'system-command',
      { command: 'ls -la' },
      undefined,
      async (toolName, params) => {
    ...
    const writeResult = await agentGuard.executeTool(
      'file-write',
    ...
    // Get audit trail
    console.log('\n6. Audit Trail');
    const auditTrail = await agentGuard.getAuditTrail({ limit: 5 });
