 ...
 * 
 * This example shows advanced features of the AgentGuard SDK
 */
...
class AdvancedAgentExample {
  ...
  constructor(config: Partial<AgentGuardConfig>) {
    this.agentGuard = new AgentGuard({
      ...config,
      debug: true
    });
  }
  ...
  async evaluateOnly(): Promise<void> {
    ...
    try {
      const decision = await this.agentGuard.evaluateTool(
        'database-write',
        { 
          table: 'users', 
  ...
  async customToolExecution(): Promise<void> {
    ...
    try {
      const result: ToolExecutionResult<CustomToolResult> = await this.agentGuard.executeTool(
        'web-search',
      ...
      if (isAgentGuardError(error)) {
        console.error('AgentGuard Error:', {
          code: error.code,
  ...
  async policyManagement(): Promise<void> {
    ...
    try {
      ...
      const policies = await this.agentGuard.getPolicies();
      ...
      const validation = await this.agentGuard.validatePolicies([customPolicy]);
  ...
  async auditAnalysis(): Promise<void> {
    ...
    try {
      const auditTrail = await this.agentGuard.getAuditTrail({ limit: 10 });
  ...
  async statisticsMonitoring(): Promise<void> {
    ...
    const stats = this.agentGuard.getStatistics();
  ...
  async runAll(): Promise<void> {
    console.log('🚀 AgentGuard SDK - Advanced Usage Examples');
    console.log('===========================================');
...
async function main(): Promise<void> {
  const config: Partial<AgentGuardConfig> = {
