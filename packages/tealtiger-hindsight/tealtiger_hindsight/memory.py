...
class HindsightGovernanceMemory:
    ...
    Args:
        ...
        metadata_fields: Which decision fields to include as Hindsight metadata
            for filtering during recall. Default includes agent_id, mode, action.
    ...
    Example:
        ...
        >>> memory.store({"action": "DENY", "tool_name": "send_email", ...})
        >>> past = memory.recall(agent_id="agent-1", context="tool:send_email")
    """
    ...
    def store(self, decision: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Args:
            decision: TealTiger governance decision dict containing at minimum:
                ...
                - Optional: agent_id, tool_name, reason_codes, risk_score, etc.
        ...
        # Store in Hindsight with importance weighting
    ...
    def recall(
        ...
        Retrieves contextually relevant past decisions. Useful for:
        - Detecting recurring denial patterns for an agent
        - Understanding baseline behavior before anomaly detection
        - Providing governance context to operators

        Args:
            agent_id: Filter by agent (uses metadata filter).
            context: Contextual query (e.g., "tool:web_search", "pii:email").
            query: Free-text semantic query for Hindsight recall.
            limit: Maximum number of memories to return.
            tags: Additional tags to filter by.
            min_importance: Minimum importance threshold for results.
    ...
    def reflect(
        ...
        Uses Hindsight's reflect operation to generate insights from stored
        governance decisions. Useful for:
        - "What are the most common denial reasons for this agent?"
        - "Has this agent's behavior changed over time?"
        - "What governance patterns should I be aware of?"

        Args:
            agent_id: Filter reflection to a specific agent.
            query: Reflection query (default: general governance summary).

        Returns:
            Synthesized reflection text from Hindsight.
        """
        reflect_query = query or f"Summarize governance patterns for agent {agent_id or 'all agents'}"
    ...
    def _format_content(self, decision: Dict[str, Any]) -> str:
        """Format a governance decision as human-readable content for Hindsight."""
